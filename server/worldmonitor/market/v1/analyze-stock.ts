...
export async function fetchDividendProfile(symbol: string, currentPrice: number): Promise<DividendProfile> {
  try {
    ...
    const currentYear = new Date().getFullYear();
    const fullYears = [...annualCounts.keys()].filter(y => y < currentYear);
    const yearsForFrequency = fullYears.length > 0 ? fullYears : [...annualCounts.keys()];
    const avgPaymentsPerYear = yearsForFrequency.length > 0
      ? yearsForFrequency.reduce((sum, y) => sum + (annualCounts.get(y) ?? 0), 0) / yearsForFrequency.length
      : 0;
    const paymentsPerYear = Math.max(recentDivs.length, avgPaymentsPerYear);
