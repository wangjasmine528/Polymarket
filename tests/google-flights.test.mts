...
function relayUrlFrom(requestedUrls: string[], path: string): URL {
  const url = requestedUrls.find((value) => value.includes(path));
  assert.ok(url, `expected relay request containing ${path}, got: ${requestedUrls.join(', ' )}`);
  return new URL(url);
}
...
describe('searchGoogleFlights — multi-airline filtering', () => {
  ...
  it('forwards multiple airlines as repeated query params to the relay', async () => {
    ...
    const url = relayUrlFrom(requestedUrls, '/google-flights/search?');
...
describe('searchGoogleDates — multi-airline filtering', () => {
  ...
  it('forwards multiple airlines as repeated query params to the relay', async () => {
    ...
    const url = relayUrlFrom(requestedUrls, '/google-flights/search-dates?');
    const airlines = url.searchParams.getAll('airlines');
